function [GCMat time2use freq2use timeOrder bicMat] = jkGetGranger(sig1, sig2, SF, mainEOI_Idx, FreqRange, FreqScale)
...
%Define time space
SWStepFactor = 10;	%1/2 of szSW in V1

stEOI = floor(nanmin(mainEOI_Idx) / (1000 / SF)); edEOI = floor(nanmax(mainEOI_Idx) / (1000 /SF));
leftBuffer = stEOI - 1;
rightBuffer = size(sig1, 2) - edEOI + 1;

if leftBuffer <= rightBuffer
    thisBuff = leftBuffer;          %# of cols
else
    thisBuff = rightBuffer;
end %leftBuffer <= rightBuffer

msecEOI = 250;
thisBuffinMsec = thisBuff * (1000 / SF);
if thisBuffinMsec < (msecEOI / 2)             %Each analysis run of Granger should be done with > 375 ms time window
    error(['Should have more buffer before and after the main signals of interests']);
end %thisBuffinMsec < (msecEOI / 2)             %Each analysis run of Granger should be done with > 375 ms time window

szEOI = round(msecEOI / (1000 / SF));
nDatInTrial = szEOI + (diff(mainEOI_Idx) + 1);
dat2use = [(stEOI - (szEOI / 2)) (edEOI + (szEOI / 2))];

sig1 = sig1(:, dat2use(1):dat2use(2));
sig2 = sig2(:, dat2use(1):dat2use(2));

timeSpace = linspace(-1 * msecEOI / 2, (diff(mainEOI_Idx) + 1) * (1000 / SF) + (msecEOI / 2), nDatInTrial);
time2use = [0 (diff(mainEOI_Idx) + 1) * (1000 / SF)];
time2useIdx = dsearchn(transpose(timeSpace), transpose(time2use));

nDivTimeSpace = 10;
time2alz = time2useIdx(1):round(szEOI/nDivTimeSpace):time2useIdx(2);			%current resolution is 300/nDivTimeSpace ms
time2use = linspace(time2use(1), time2use(2), length(time2alz));
...
minBIC = 5; maxBIC = floor(szEOI * .50); nBicSpace = 5; bicMat = nan(length(time2alz), nBicSpace);
if maxBIC <= minBIC
...
for timeRUN = 1:1:length(time2alz)
	dat1_t = sig1(:, (time2alz(timeRUN) - szEOI / 2):(time2alz(timeRUN) + szEOI / 2 - 1));
	dat2_t = sig2(:, (time2alz(timeRUN) - szEOI / 2):(time2alz(timeRUN) + szEOI / 2 - 1));
